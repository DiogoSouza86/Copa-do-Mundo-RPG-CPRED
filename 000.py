...
if fn1 > fn2:
    ...
    gols_penal_t1 = []
    gols_penal_t2 = []
    lista_erro_penal = ['Pra fora!', 'Goleiro defende!', 'Chute fraco, cade o feijão?', 'Na traaave!',
                            'Rebote do goleiro!!!']
    for r, chute in enumerate(range(1,11)):

        sleep(1)
        #se o chute for par o time 1 cobra
        if chute % 2 == 0:
            print(f'{lista_final[0]} chutou...')
            sleep(3)
            rola_d6 = [1,2,3,4,5,6]
            d6 = choice(rola_d6)
            #se o d6 cair par o atacante marca
            if d6 % 2 == 0:
                gols_penal_t1.append(1)
                print(f'Gooool da {lista_final[0]}')
                sleep(2)
            #se cair impar o atacante erra
            else:
                erro_penal = choice(lista_erro_penal)
                print(erro_penal)
                sleep(2)
                #se o goleiro der rebote o time 1 tem um nova chance de marcar
                if erro_penal == 'Rebote do goleiro!!!':
                    print("A bola ta viva!!!!!")
                    sleep(3)
                    rola_d6 = [1, 2, 3, 4, 5, 6]
                    d6 = choice(rola_d6)
                    #se o d6 cair acima ou igual a 5 o time atacante chega na bola
                    if d6 >= 5:
                        print(f'{lista_final[0]} pegou o rebote e...')
                        rola_d6 = [1, 2, 3, 4, 5, 6]
                        d6 = choice(rola_d6)
                        #se der 6 no d6 o time atacante marca um gol
                        if d6 == 6:
                            gols_penal_t1.append(1)
                            print(f'Gooool da {lista_final[0]}')
                            sleep(2)
                        else:
                            lista_rebote_penal = ["Chutou pra fora!", "Rebote recuperado pelo goleiro!", "Chuta pro gol mas o goleirao defende"]
                            rebote_penal = choice(lista_rebote_penal)
                            print(rebote_penal)
                    #se o d6 cair abaixo de 4 o time atacante não pega o rebote
                    else:
                        print(f'Rebote perdido...')
                        sleep(2)
        #se o chute for impar o time 2 cobra
        else:
            print(f'{lista_final[1]} chutou...')
            sleep(3)
            rola_d6 = [1, 2, 3, 4, 5, 6]
            d6 = choice(rola_d6)
            if d6 % 2 == 0:
                gols_penal_t2.append(1)
                print(f'Gooool da {lista_final[1]}')
                sleep(2)
            else:
                erro_penal = choice(lista_erro_penal)
                print(erro_penal)
                sleep(2)
        gols_p_t1 = sum(gols_penal_t1)
        gols_p_t2 = sum(gols_penal_t2)
        print(f'>>{r + 1} ----{gols_p_t1} para {lista_final[0]}')
        print(f'>>--------{gols_p_t2} para {lista_final[1]}')
    #caso ocora empate nos penal
    if gols_p_t1 == gols_p_t2:
        print('Empate nos penalties, quem fizer ganha.')
        desempate_penal = True
        while desempate_penal:
            print(f'{lista_final[0]} chutou...')
            sleep(3)
            rola_d6 = [1, 2, 3, 4, 5, 6]
            d6 = choice(rola_d6)
            if d6 % 2 == 0:
                gols_penal_t1.append(1)
                print(f'Gooool da {lista_final[0]}')
                print("Acabou, acabou, acabou, acabou...")
                break
            else:
                print("Errou")
                sleep(3)
    ...
    print(f'\n{lista_final[0]} marcou {gols_p_t1}')
    print(f'\n{lista_final[1]} marcou {gols_p_t2}')